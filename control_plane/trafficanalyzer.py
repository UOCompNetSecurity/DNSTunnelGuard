...
class TrafficDNSAnalyzer(DNSAnalyzer):
    ...
    def __init__(
        ...
        assert (
            num_queries_for_domain_threshold > 0 and num_queries_from_ip_threshold > 0
        )
    ...
    def analyze(self, dns_event_query: RecordEvent) -> float:
        ...
        for domain in domains:
            ...
            max_domain_sus_percentage = max(
                domain_sus_percentage, max_domain_sus_percentage
            )
    ...
    def _reap_old_queries(self, domains: list[str], ip_address: str):
        ...
        get_next_timestamp = lambda history, key: (
            history[key][0] if history[key] else None
        )
        is_old = (
            lambda now, timestamp, threshold: (now - timestamp).total_seconds() / 60
            > threshold
        )

        for domain in domains:
            domain_timestamp = get_next_timestamp(self.domain_history, domain)
            while domain_timestamp and is_old(
                now, domain_timestamp, self.domain_minute_difference_threshold
            ):
                self.domain_history[domain].popleft()
                domain_timestamp = get_next_timestamp(self.domain_history, domain)

        ip_timestamp = get_next_timestamp(self.ip_history, ip_address)
        while ip_timestamp and is_old(
            now, ip_timestamp, self.ip_minute_difference_threshold
        ):
